...
const Medicos = ({medicos, setMedicos}) => {
    ...
    const handleEditMedico = (id) => {
        navigate(`/editar-medicos/${id}`);
    };

    const handleViewMedicosDetalles = () => {
        navigate('/medicos-detalles', { state: { medicos: filteredMedicos } });
    };
    ...
    const handleDeleteMedico = (id) => {
        if (window.confirm('¿Estás seguro de que deseas eliminar este médico?')) {
            const deleteMedico = async () => {
                try {
                    const response = await fetch('http://localhost/sisDenatal/backend2/public/index.php?action=deletemedico', {
                        method: 'POST',
                        headers: {
                    ...
                    const data = await response.json();
                    setMedicos(medicos.filter(medicos => medicos.id !== id));
                    setFilteredMedicos(medicos.filter(medicos => medicos.id !== id));
                } catch (error) {
                    console.error('Error:', error);
                }
            }
                    ...
                    <tbody>
                        {filteredMedicos.map(medico => (
                            <tr key={medico.id}>
                                ...
                                <td>
                                    <button className="btn-edit-m" onClick={() => handleEditMedico(medico.id)}>Editar</button>
                                    <button className="btn-delete-m" onClick={() => handleDeleteMedico(medico.id)}>Eliminar</button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
