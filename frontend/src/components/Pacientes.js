    ...
    const handleDeletePaciente = (id) => {
        if (window.confirm('¿Estás seguro de que deseas eliminar este paciente?')) {
            const deletePaciente = async () => {
                try {
                    const response = await fetch('http://localhost/sisDenatal/backend2/public/index.php?action=deletepaciente', {
                        method: 'POST',
                        headers: {
                    ...
                    const data = await response.json();
                    console.log(data);
                    setPacientesData(pacientesData.filter(pacientesData => pacientesData.id !== id));
                    setFilteredPacientes(pacientesData.filter(pacientesData => pacientesData.id !== id));
                } catch (error) {
                    console.error('Error:', error);
                }
            }
        ...
        <div className="usuarios-body">
            ...
            {isLoading ? (
                ...
                <div className="usuarios-container">
                            ...
                            {pacientesData
                                ...
                                .map(paciente => (
                                    <tr key={paciente.id}>
                                        <td>
                                            {paciente.photo ?
                                                <img src={paciente.photo} alt={paciente.nombre} className="user-photo" /> :
                                        ...
                                        <td>
                                            <div className="opciones-container">
                                                <button className="btn-view-p" onClick={() => handleViewHistorial(paciente.id)}>Ver</button>
                                                <button className="btn-editt" onClick={() => handleEditPaciente(paciente.id)}>Editar</button>
                                                <button className="btn-deletee" onClick={() => handleDeletePaciente(paciente.id)}>Eliminar</button>
                                            </div>
                                        </td>
                                    </tr>
                                ))}
                    ...
                    {selectedPaciente && <HistorialClinicoModal paciente={selectedPaciente} onClose={closeModal} />}
                </div>
            )}
        </div>
    );
