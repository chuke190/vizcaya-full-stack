...
const Odontograma = () => {
    ...
    const handlePatientChange = (e) => {
        ...
        const fetchOdontograma = async () => {
            try {
                const response = await fetch(`http://localhost/sisDenatal/backend2/public/index.php?action=getodontograma`, {
                    method: 'POST',
                    headers: {
    ...
    const handleExport = () => {
        if (odontogramaData) {
            const blob = new Blob([odontogramaData], { type: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' });
    ...
    const handleSave = async () => {
        if (selectedPatient && odontogramaData) {
            const response = await fetch('http://localhost/sisDenatal/backend2/public/index.php?action=uploadfile', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
        ...
        <div className="odontograma-body">
            ...
            <div className="odontograma-container">
                ...
                <div className="button-group">
                    ...
                    <input type="file" id="file-upload" style={{ display: 'none' }} onChange={handleImport} />
