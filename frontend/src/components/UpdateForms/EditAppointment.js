...
const EditAppointment = ({
  ...
  useEffect(() => {
    if (citaToEdit) {
      ...
      const paciente = pacientesData.find(
        (paciente) => paciente.nombre === citaToEdit.paciente
      );
      setPacienteSeleccionado(paciente);
      const tratamiento = tratamientos.find(
        (trat) => trat.tratamiento === citaToEdit.tratamiento
      );
  ...
  useEffect(() => {
    if (citaData.paciente) {
      const paciente = pacientesData.find(
        (paciente) => paciente.nombre === citaData.paciente
      );
      setPacienteSeleccionado(paciente);

      if (paciente) {
        // Buscar la cita relacionada con el paciente seleccionado
        const citaRelacionada = citasData.find(
          (cita) =>
            cita.paciente === paciente.nombre && cita.id !== parseInt(id)
        );
        if (citaRelacionada) {
          const tratamiento = tratamientos.find(
            (trat) => trat.tratamiento === citaRelacionada.tratamiento
          );
          setTratamientoSeleccionado(tratamiento);
          if (tratamiento) {
  ...
  const handleTratamientoChange = (e) => {
    ...
    const tratamiento = tratamientos.find(
      (trat) => trat.id === parseInt(tratamientoId)
    );
    if (tratamiento) {
      setTratamientoSeleccionado(tratamiento);
      setCitaData((prevState) => ({
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    setCitasData((prevState) =>
      prevState.map((cita) => (cita.id === parseInt(id) ? citaData : cita))
    );
    navigate("/citas");
  };
          ...
          <div className="form-table">
            ...
            <div className="infocit">
              {pacienteSeleccionado && (
                <div className="form-group paciente-info">
                  ...
                  <p>
                    <strong>Estado:</strong> {citaData.estado}
                  </p>
                  {tratamientoSeleccionado && (
                    <>
                      ...
                      <p>
                        <strong>Costo:</strong> {citaData.costo}
                      </p>
                    </>
                  )}
                </div>
            ...
            <div className="form-group">
              ...
              <select
                ...
                name="tratamiento"
                value={
                  tratamientoSeleccionado ? tratamientoSeleccionado.id : ""
                }
