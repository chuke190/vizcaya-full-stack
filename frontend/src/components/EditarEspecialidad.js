...
const EditarEspecialidad = ({ especialidadesData, setEspecialidadesData }) => {
    ...
    useEffect(() => {
        const especialidad = especialidadesData.find(especialidad => especialidad.id === parseInt(id));
        if (especialidad) {
    ...
    const handleSubmit = async (e) => {
        ...
        const response = await fetch(`http://localhost/sisDenatal/backend2/public/index.php?action=updateespecialidad`, {
            method: 'PUT',
            headers: {
        ...
        if (data.message === 'exito') {
            const updatedEspecialidadesData = especialidadesData.map(especialidad => {
