...
const Especialidades = ({ especialidadesData, setEspecialidadesData }) => {
    ...
    const handleDeleteEspecialidad = (id) => {
        if (window.confirm('¿Estás seguro de que deseas eliminar esta especialidad?')) {
            const deleteEspecialidad = async () => {
                try {
                    const response = await fetch('http://localhost/sisDenatal/backend2/public/index.php?action=deleteespecialidad', {
                        method: 'POST',
                        headers: {
                    ...
                    const data = await response.json();
                    console.log(data);
                    setEspecialidadesData(especialidadesData.filter(especialidad => especialidad.id !== id));
                } catch (error) {
                    console.error('Error:', error);
                }
            }
    ...
    const handleDeleteSelected = () => {
        if (window.confirm('¿Estás seguro de que deseas eliminar las especialidades seleccionadas?')) {
            
        }
    };

    const handleDeleteAll = () => {
        if (window.confirm('¿Estás seguro de que deseas eliminar todas las especialidades?')) {
            const deleteAllEspecialidades = async () => {
                try {
                    const response = await fetch('http://localhost/sisDenatal/backend2/public/index.php?action=dropallesp', {
                        method: 'DELETE'
                        ...
                        {especialidadesData
                            ...
                            .map((especialidad, index) => (
                                <tr key={especialidad.id}>
                                    ...
                                    <td>
                                        <button className="botn-edit" onClick={() => handleEditEspecialidad(especialidad.id)}>Editar</button>
                                        <button className="botn-delete" onClick={() => handleDeleteEspecialidad(especialidad.id)}>Eliminar</button>
                                    </td>
                                </tr>
                            ))}
                    </tbody>
