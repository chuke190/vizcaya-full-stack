...
import Nav from './Nav';
import logoo from '../img/15.png';

const RecetaPaciente = ({ medicos, setMedicos }) => {
  const [patientName, setPatientName] = useState('');
  const [fechanac, setFechanac] = useState('');
  const [medicines, setMedicines] = useState([{ name: '', dosage: '', schedule: '' }]);
  const [instructions, setInstructions] = useState('');
  const [sexo, setSexo] = useState('');
  const [edad, setEdad] = useState('');
  const [selectedMedico, setSelectedMedico] = useState(medicos.length > 0 ? medicos[0] : null);
  const [selectedMedicoDNI, setSelectedMedicoDNI] = useState(medicos.length > 0 ? medicos[0].dni : '');
  ...
  const handleMedicoChange = (e) => {
    const selected = medicos.find(medico => medico.id === parseInt(e.target.value));
  ...
  const handleDownload = () => {
    const text = `
      ...
      Medicamentos:
      ${medicines.map((medicine) => `${medicine.name} - ${medicine.dosage} - ${medicine.schedule}`).join('\n')}
    ...
    <div className="receta-body">
      ...
      <div className="receta-container">
        <div className="receta-content">
          ...
          <form className="receta-form">
            ...
            <div className="info">
              {medicines.map((medicine, index) => (
                <div key={index} className="medicine-group">
                  ...
                  <button className="btn-del" type="button" onClick={() => removeMedicine(index)}>Eliminar</button>
                </div>
              ))}
            </div>
            <button className="btn-adde" type="button" onClick={addMedicine}>Añadir Medicamento</button>
            ...
            <div className="form-group">
              ...
              <select id="medico" name="medico" value={selectedMedico?.id || ''} onChange={handleMedicoChange}>
                {medicos.map((medico) => (
            ...
            <div className="logoo-container">
              ...
              <div className="subtitulos-receta">
                <div className="subtiit">
                  <strong className="sub">{selectedMedico?.nombre.toUpperCase()}</strong>
                  <p className="sub">MEDICO CIRUJANO CED. PROF.{selectedMedicoDNI}</p>
                  <p className="sub">UNIVERSIDAD......................</p>
                </div>
                <p className="dire">Clinica Dental Vizcaya Calle Benito Crespo 233 entre Fúnez Vásquez de Mercado, Colonia Nueva Vizcaya, 34080 Durango, Dgo.</p>
              </div>
              <p className="logoo-container-fecha-hora">FECHA Y HORA DE ELABORACIÓN: {fechaActual} {horaActual}</p>

            </div>
            <div className="container-form-sub">
              <p><strong>Médico:{selectedMedico?.nombre} DNI: {selectedMedicoDNI}</strong></p>
            </div>
            <div className="container-form">
              <p><strong>NOMBRE:</strong> {patientName}</p>
              <p><strong>SEXO:</strong> {sexo}____________<strong>EDAD:</strong> {edad}____________<strong>FECHA DE NACIMIENTO:</strong> {fechanac}</p>
              <p><strong>MEDICAMENTOS:</strong></p>
              <ul>
                {medicines.map((medicine, index) => (
                  <li key={index}>
                    <strong>{medicine.nombre}</strong> - {medicine.dosage} cada {medicine.schedule} hrs
                  </li>
                ))}
              </ul>
              <p><strong>INSTRUCCIONES GENERALES:</strong><br />
                {instructions}
              </p>
              <br />
              <br />
              <h3><strong>FIRMA: ____________________________</strong></h3>
        ...
        <div className="btn-group">
          <button className="buttonIm" type="button" onClick={handlePrint}>Imprimir</button>
