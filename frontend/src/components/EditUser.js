...
const EditUser = ({ users, setUsers }) => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!userData.nombre || !userData.rol || !userData.noDocumento || !userData.email || !userData.tipoDocumento || !userData.telefono) {
        ...
        const response = await fetch('http://localhost/sisDenatal/backend2/public/index.php?action=updateuser', {
            method: 'PUT',
            headers: {
                ...
                <form onSubmit={handleSubmit} className="rect">
                    <div className="form-tablee">
                        ...
                        <div className="form-group">
                            ...
                            <input type="text" id="tipoDocumento" name="tipoDocumento" value={userData.tipoDocumento} onChange={handleChange} required />
                        </div>
                        <div className="form-group">
                            <label htmlFor="noDocumento">No. Documento:</label>
                            <input type="number" id="noDocumento" name="noDocumento" value={userData.noDocumento} onChange={handleChange} required />
                        </div>
                        <div className="form-group">
                            <label htmlFor="telefono">Teléfono:</label>
                            <input type="tel" id="telefono" name="telefono" value={userData.telefono} onChange={handleChange} required />
                        </div>
                        <div className="form-group">
