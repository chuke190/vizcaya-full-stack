...
import Nav from "../components/Nav"; // Asegúrate de que esta línea está presente
...
const Tratamientos = ({ tratamientosData, setTratamientosData }) => {
  ...
  const handleDeleteTratamiento = async (id) => {
    if (
      ...
      if (data.message === "exito") {
        const newTratamientosData = tratamientosData.filter(
  ...
  const handleDeleteSelected = () => {
    if (
      window.confirm(
        "¿Estás seguro de que deseas eliminar los tratamientos seleccionados?"
      )
    ) {
      const newTratamientosData = tratamientosData.filter(
        (tratamiento) => !selectedTratamientos.includes(tratamiento.id)
      );
      setTratamientosData(newTratamientosData);
  ...
  const handleDeleteAll = () => {
    if (
      window.confirm(
        "¿Estás seguro de que deseas eliminar todos los tratamientos?"
      )
    ) {
      setTratamientosData([]);
    }
                ...
                <tr key={tratamiento.id}>
                  ...
                  <td>
                    <button
                      ...
                      onClick={() => handleEditTratamiento(tratamiento.id)}
                    >
                      Editar
                    </button>
                    <button
